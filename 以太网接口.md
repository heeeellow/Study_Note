...
#### **核心痛点深度剖析：Skew (时钟偏斜)与 Timing Budget**
...
| :---- | :---- | :---- | :---- |
| "rgmii" | No Internal Delay | MAC输出的时钟和数据是对齐的。**必须依靠PCB走线**（时钟线比数据线绕得长）来实现延时。 | 历史遗留硬件，或老式PHY芯片。 |
| "rgmii-id" | **Internal Delay** | **推荐配置**。PHY芯片内部在TX和RX路径上开启2ns的数字延时器。PCB可以直连，无需蛇形走线。 | 现代设计首选。 |
...
## 第三部分：管理接口 (Control Path) \- MDIO/SMI
...
## 第四部分：BSP 实战 —— Device Tree 与 Driver 深度配置
